...
def evaluate_detector_all(dataset, logistic_model_aie, linear_model_aie, mlp_regressor, mlp_classifier, item=None):
    ...
    acc1, auc1, tpr1 = evaluate_detector(all_aies, all_labels, model=logistic_model_aie)
    print("->linear_model_aie")
    acc2, auc2, tpr2 = evaluate_detector(all_aies, all_labels, model=linear_model_aie)

    print("->mlp_regressor")
    acc3, auc3, tpr3 = evaluate_detector(all_aies, all_labels, model=mlp_regressor)
    print("->mlp_classifier")
    acc4, auc4, tpr4 = evaluate_detector(all_aies, all_labels, model=mlp_classifier)

    return acc3, auc3, tpr3
...
def evaluate_detector_combine(dataset, model_name, task, model_1, model_2):
    ...
    tn, fp, fn, tp = confusion_matrix(Y_test, combined_predictions).ravel()
    combine_tpr = tp / (tp + fn)

    return combined_accuracy, combined_auc, combine_tpr
...
if __name__ == '__main__':
    ...
    '''
    datasets = [["Badnet()", "Badnet_test()"], ["CTBA()", "CTBA_test()"], ["MTBA()", "MTBA_test()"], ["Sleeper()", "Sleeper_test()"], ["VPI()", "VPI_test()"]]
    
    task = 'lie'
    dataset_str = "Questions1000()"
    # dataset_test_str = "Badnet_test()"
    model_names = ["Llama-2-7b-chat-hf",  "Llama-2-13b-chat-hf", "Meta-Llama-3.1-8B-Instruct", "Mistral-7B-Instruct-v0.2"]
    ...
    # ======================================== Bias detection task ========================================
        ...
        pattern = r"category='([^']*)'"
        match = re.search(pattern, dataset_str)
        if match:
    ...
    # ------------------------------------------ Backdoor detection task -------------------------------------------
        ...
        layer_results = []
        prompt_results = []
        for dataset_str, dataset_test_str in datasets:
            dataset = eval(dataset_str)
            print("-->dataset num:", len(dataset), len(dataset.columns))
            dataset_name = dataset.__class__.__name__
            # print("-->dataset.columns", dataset.columns)
            ...
            test_dataset = eval(dataset_test_str)
            ...
            one_layer_results = [] # for one dataset
            one_prompt_results = [] # for one dataset
            one_combine_results = [] # for one dataset
            for model_name in model_names:
                ...
                logistic_model_aie1, linear_model_aie1, mlp_regressor1, mlp_classifier1 = train_detector(train_dataset,
                                                                                                         ...
                                                                                                         target='layer',
                                                                                                         lie_instruction_num='random')
                acc, auc, tpr = evaluate_detector_all(dataset=test_dataset, model_name=model_name, task=task,
                                          logistic_model_aie=logistic_model_aie1, linear_model_aie=linear_model_aie1,
                                          mlp_regressor=mlp_regressor1, mlp_classifier=mlp_classifier1,
                                          target='layer')
                one_layer_results.append(acc)
                one_layer_results.append(auc)
                ...
                acc, auc, tpr = evaluate_detector_all(dataset=test_dataset, model_name=model_name, task=task,
                                          ...
                                          mlp_regressor=mlp_regressor2, mlp_classifier=mlp_classifier2,
                                          target='prompt')
                one_prompt_results.append(acc)
                one_prompt_results.append(auc)


                combined_acc, combined_auc, combined_tpr = evaluate_detector_combine(test_dataset, model_name, task, mlp_regressor1, mlp_regressor2)
                one_combine_results.append(combined_acc)
                one_combine_results.append(combined_auc)
                # one_combine_results.append(combined_tpr)

            layer_results.append(one_layer_results)
            prompt_results.append(one_prompt_results)
            combine_results.append(one_combine_results)
    
        df = pd.DataFrame(layer_results, columns=['Acc', 'Auc', 'Acc', 'Auc', 'Acc', 'Auc', 'Acc', 'Auc'])
        df.to_csv('results_layer.csv', index=False)  # 'index=False' prevents pandas from writing row indices

        df = pd.DataFrame(prompt_results, columns=['Acc', 'Auc', 'Acc', 'Auc', 'Acc', 'Auc', 'Acc', 'Auc'])
        df.to_csv('results_prompt.csv', index=False)  # 'index=False' prevents pandas from writing row indices

        df = pd.DataFrame(combine_results, columns=['Acc', 'Auc', 'Acc', 'Auc', 'Acc', 'Auc', 'Acc', 'Auc'])
        df.to_csv('results_combine.csv', index=False)  # 'index=False' prevents pandas from writing row indices




    # ------------------------------------------ Backdoor detection task (generalization)-------------------------------------------
    elif task == '':
        combine_results = []
        layer_results = []
        prompt_results = []
        ...
        for dataset_str, dataset_test_str in datasets:
            ...
            # 分割数据集
            train_df, test_df = train_test_split(dataset, test_size=0.3, random_state=1)  # 确保可复现性

            print("Training set size:", len(train_df))
            print("Testing set size:", len(test_df))

            # 添加到总的训练集和测试集DataFrame
            train_dataset = pd.concat([train_dataset, train_df], ignore_index=True)
            test_dataset = pd.concat([test_dataset, test_df], ignore_index=True)

            # 在循环结束后，你将得到包含所有数据集训练数据的all_train_df和测试数据的all_test_df
            print("Final training dataset size:", len(train_dataset))
            print("Final testing dataset size:", len(test_dataset))
        ...
        # df = pd.DataFrame(layer_results, columns=['Acc', 'Auc', 'TPR', 'Acc', 'Auc', 'TPR', 'Acc', 'Auc', 'TPR', 'Acc', 'Auc', 'TPR'])
        df = pd.DataFrame(layer_results, columns=['Acc', 'Auc', 'Acc', 'Auc', 'Acc', 'Auc', 'Acc', 'Auc'])
        df.to_csv('results_layer.csv', index=False)  # 'index=False' prevents pandas from writing row indices

        # df = pd.DataFrame(prompt_results, columns=['Acc', 'Auc', 'TPR', 'Acc', 'Auc', 'TPR', 'Acc', 'Auc', 'TPR', 'Acc', 'Auc', 'TPR'])
        df = pd.DataFrame(prompt_results, columns=['Acc', 'Auc', 'Acc', 'Auc', 'Acc', 'Auc', 'Acc', 'Auc'])
        df.to_csv('results_prompt.csv', index=False)  # 'index=False' prevents pandas from writing row indices

        # df = pd.DataFrame(combine_results, columns=['Acc', 'Auc', 'TPR', 'Acc', 'Auc', 'TPR', 'Acc', 'Auc', 'TPR', 'Acc', 'Auc', 'TPR'])
        df = pd.DataFrame(combine_results, columns=['Acc', 'Auc', 'Acc', 'Auc', 'Acc', 'Auc', 'Acc', 'Auc'])
        df.to_csv('results_combine.csv', index=False)  # 'index=False' prevents pandas from writing row indices
